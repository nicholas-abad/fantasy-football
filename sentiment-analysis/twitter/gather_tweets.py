...
def search_twitter(search_term, limit):
    ...
    try:
        ...
        # Drop twitter API connection
        # del twitter
        print("Because of this, the application will sleep for %d seconds" % remainder)
        # Pause until we can go again.
        time.sleep(remainder)
        print('Done sleeping!')
        print('Retrying code!')
        search_twitter(search_term, limit)
...
if __name__ == "__main__":
    # Gather the tweets.
    relevant_tweets, relevant_tweets_num_retweets, relevant_tweets_num_favorites = search_twitter(search_term, num_tweets)
    print(f'Number of relevant tweets before deleting duplicates: {len(relevant_tweets)}')

    # Transform tweets into a dataframe.
    tweets_df = to_pd_dataframe(relevant_tweets, relevant_tweets_num_retweets, relevant_tweets_num_favorites)
