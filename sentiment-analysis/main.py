...
def update_dataframe_with_sentiment_scores(dataframe, name_of_players_column):
    ...
    for idx, name in enumerate(player_names):
        path_to_tweets = (
            f"./sentiment-analysis/twitter/output/16-09-2020_{name}_tweets.csv"
        )

        if os.path.exists(path_to_tweets):
            player_sentiment_score, num_tweets = _get_sentiment_analysis_on_dataframe(
                path_to_tweets
            )
...
if __name__ == "__main__":
    sentiment_dataframe = update_dataframe_with_sentiment_scores(
        original_data, "Player_Names"
    )
    sentiment_dataframe.to_csv(
        "./data/fantasy_football_auction_values_with_sentiment.csv", index=False
    )
